...
Plan C as-shipped includes the v2.4 (suggest-only + SS-primary), v2.5 (MCP client + open-webSearch + paperhub-papers FastMCP), v2.6 stabilisation, v2.7 (four-stage paper_search decomposition + opt-in CUDA + device auto-detect), v2.8 (model server isolation), v2.9 (PDF upload + arXiv-ID manual import), v2.10 (agentic hierarchical paper_qa + chunker hardening + agent-flow observability), and v2.11 (router context dispatch) rounds. v2.10 rebuilt `paper_qa` into a per-paper-subagent + finalizer subgraph (section-TOC navigation via `list_sections`/`read_section`), added the `paper_content.sections_json` column + `paperhub-reingest` CLI, and applied the agent-flow observability policy to both paper_qa and paper_search. v2.11 fixed a multi-turn-context regression: the history-aware **router** now resolves the latest turn's anaphora into a self-contained `resolved_query` (new `RoutingDecision` field) written to `AgentState.effective_query`; every downstream agent reads `effective_query` (via the `paperhub.agents.state.effective_query` helper, fallback to raw `user_message`) instead of the bare turn, and a new `intent="clarify"` lets the router surface a deliberate clarifying question rather than dead-ending in an empty-results re-ask. See [docs/superpowers/plans/2026-05-18-paperhub-C-paper-pipeline-research-agent.md](docs/superpowers/plans/2026-05-18-paperhub-C-paper-pipeline-research-agent.md) Plan C v2.4 / v2.5 / v2.6 / v2.7 / v2.8 / v2.9 / v2.10 / v2.11 sections.
