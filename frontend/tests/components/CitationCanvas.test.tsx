...
const server = setupServer(
  ...
  http.get(`${API_BASE_URL}/papers/content/7/document`, () => HttpResponse.json({ mode: "html" })),
  http.get(`${API_BASE_URL}/papers/content/8/document`, () => HttpResponse.json({ mode: "html" })),
);
...
describe("CitationCanvas reading panel", () => {
  ...
  it("opening via citation resolves the chunk and shows that paper (html)", async () => {
    ...
    await waitFor(() => expect(iframe).toHaveAttribute("src", `${API_BASE_URL}/papers/content/7/html`));
    expect(iframe).toHaveAttribute("sandbox", "allow-scripts allow-same-origin");
  });

  it("renders a switcher tab per enabled reference; clicking switches the paper", async () => {
    render(<CitationCanvas />);
    ...
    await waitFor(() =>
      expect(screen.getByTitle(/citation canvas/i)).toHaveAttribute("src", `${API_BASE_URL}/papers/content/8/html`),
    );
  });

  it("shows the source PDF (not html) for a pdf-rendered paper", async () => {
    server.use(http.get(`${API_BASE_URL}/papers/content/7/document`, () => HttpResponse.json({ mode: "pdf" })));
    render(<CitationCanvas />);
    act(() => useCanvasStore.getState().openCitation(42));
    const iframe = await screen.findByTitle(/citation canvas/i);
    await waitFor(() => expect(iframe).toHaveAttribute("src", `${API_BASE_URL}/papers/content/7/pdf`));
    expect(await screen.findByText(/source PDF|isn't available for PDF/i)).toBeInTheDocument();
  });
