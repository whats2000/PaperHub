...
async def test_decks_table_exists_with_unique_session(migrated_db: aiosqlite.Connection) -> None:
    ...
    await migrated_db.execute(
        "INSERT INTO decks (session_id, tex_path, page_count, contributing_paper_ids_json) "
        "VALUES (1, 'slides/deck.tex', 0, '[]')"
    )
    await migrated_db.commit()
...
async def test_decks_status_check(migrated_db: aiosqlite.Connection) -> None:
    ...
    with pytest.raises(aiosqlite.IntegrityError):
        await migrated_db.execute(
            "INSERT INTO decks (session_id, tex_path, page_count, contributing_paper_ids_json, status) "
            "VALUES (1, 'x', 0, '[]', 'bogus')"
        )
