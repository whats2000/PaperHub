...
# ---------------------------------------------------------------------------
# POST /sessions/import — upload a browser-local chat into the DB (so the DB
# becomes the single source of truth and the chat is visible cross-device)
# ---------------------------------------------------------------------------


async def test_import_session_creates_with_messages(
    sessions_client: AsyncClient, tmp_path: Path,
) -> None:
    resp = await sessions_client.post(
        ...
        json={
            ...
            "messages": [
                {"role": "user", "content": "explain flow matching"},
                {"role": "assistant", "content": "Flow matching is..."},
            ],
        },
    )
    assert resp.status_code == 200
    assert resp.json()["session_id"] == 65

    # It now lists + replays.
    listed = await sessions_client.get("/sessions")
    assert any(s["id"] == 65 for s in listed.json())
    msgs = await sessions_client.get("/sessions/65/messages")
    assert [m["content"] for m in msgs.json()] == [
        "explain flow matching",
        "Flow matching is...",
    ]


async def test_import_session_is_idempotent_by_id(
    sessions_client: AsyncClient,
) -> None:
    """Two devices importing the same local chat (same id) must NOT duplicate
    its messages — the second import is a no-op on content."""
    body = {
        "id": 65,
        "title": "Flow matching",
        "messages": [{"role": "user", "content": "hi"}],
    }
    await sessions_client.post("/sessions/import", json=body)
    await sessions_client.post("/sessions/import", json=body)
    msgs = await sessions_client.get("/sessions/65/messages")
    assert len(msgs.json()) == 1, "messages must not be duplicated on re-import"


async def test_import_session_untombstones(
    sessions_client: AsyncClient, tmp_path: Path,
) -> None:
    """Importing a soft-deleted session revives it (the user clearly wants it)."""
    db_path = tmp_path / "paperhub.db"
    async with aiosqlite.connect(db_path) as conn:
        await conn.execute(
            "INSERT INTO chat_sessions (id, title, deleted_at) "
            "VALUES (5, 'Tombstoned', datetime('now'))",
        )
        await conn.execute(
            "INSERT INTO messages (session_id, role, content) VALUES (5, 'user', 'x')",
        )
        await conn.commit()

    resp = await sessions_client.post(
        "/sessions/import", json={"id": 5, "title": "Tombstoned", "messages": []},
    )
    assert resp.status_code == 200
    listed = await sessions_client.get("/sessions")
    assert any(s["id"] == 5 for s in listed.json()), "import must un-tombstone"
