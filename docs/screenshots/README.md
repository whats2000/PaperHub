...
Traditional-Chinese notes — shots **11–12** can be taken without regenerating.

## Shot list (demo order)

### Shell + observability
- [x] `01-app-overview.png` — full window: sidebar + chat + composer.
      *Caption:* "One chat shell; every turn routed to a specialist agent."
- [x] `02-routing-badge.png` — a turn showing the routing badge (intent + model).
      *Caption:* "Visible routing — see which agent + model handled each turn."
- [x] ★ `03-trace-panel.png` — expanded trace DAG of a turn (per-step latency/status).
      *Caption:* "Every model/MCP/pipeline step is an audit row — the full DAG replays from SQLite."

### Grounded RAG — the provenance hero
- [x] ★ `04-citation-canvas-html.png` — an answer with `[chunk:N]`, clicked, the
...
- [x] ★ `11-slides-generate.png` — deck chip (slides-only, **"Generate notes"** button)
...
- [x] ★ `12-slides-notes-added.png` — after "generate speaker notes": note pane filled;
      chip now shows "Edit notes".
      *Caption:* "Speaker notes are opt-in — authored separately from the slides."

---

**Captured set: 01–12.** Shots 13–16 from the original plan (slides re-language
before/after, slide diff-edit before/after, slides-trace, math rendering) were
dropped — their points are covered by 03 / 11 / 12. Add later if a slide needs them.

Strongest to embed in the top-level `README.md` feature section: the Citation
Canvas pair (04/05), the NL→SQL shot (09), and the slides pair (11/12).
