...
| v1.9 | 2026-05-17 | **Three-tier source-fidelity ladder (clarification, not a new requirement).** Rewrites §1.1 clause 1 to characterize the extraction tiers correctly. Earlier drafts treated Marker as either co-equal with LaTeX (wrong — Marker is post-extraction Markdown, not source) or as a separate "raw PDF" tier (also wrong — Marker IS markdown, just *equation-preserving* markdown). The corrected ladder: **Tier 1 = raw LaTeX source** (lossless, same as paper2slides-plus); **Tier 2 = Marker** (equation-preserving Markdown — equations as inline LaTeX, tables as Markdown tables, figures as image refs); **Tier 3 = raw text extraction** (`pdfminer`/`PyMuPDF`/HTML→Markdown — equations collapse to broken unicode, used only as a last-resort when Tiers 1 and 2 both fail; slide pipeline MAY refuse Tier-3 inputs). Imports record which tier produced the artifact (via `papers.notes_md` or a dedicated `papers.extraction_tier` column) so downstream consumers act on it. No FR or NFR changes; this is a writing clarification to prevent future drift. |
| v1.10 | 2026-05-17 | **Tier 1 is the unpacked e-print archive, not just flattened LaTeX; Marker becomes a containerized HTTP service.** Two corrections surfaced when reviewing the Phase A LaTeX-first implementation: **(1)** `arxiv-latex-mcp::get_paper_prompt` returns *only* the flattened LaTeX text, which is sufficient for RAG indexing but insufficient for FR-05 / FR-07 slide compilation — those need the **figures** (`.png` / `.pdf` / `.eps` files referenced by `\includegraphics`), the **bibliography** (`.bib` files), the **style files** (`.sty` / `.cls`), and any `\include{}`'d chapter files. paper2slides-plus's `arxiv_utils.py` (`copy_image_assets_from_cache`) demonstrates the canonical approach: the **primary Tier-1 artifact is the unpacked e-print archive**, not just the flattened text. §1.1 clause 1 Tier 1 updated: the import pipeline downloads the raw e-print `.tar.gz` from `https://arxiv.org/e-print/<id>`, unpacks it under `workspace_root/papers/<id>/source/`, and ALSO keeps the flattened LaTeX text (for RAG) at `.../source.flattened.tex`. **(2)** Marker (Tier 2) is too heavy to install as a PaperHub Python dependency (~2–4 GB ML models, plus PyTorch + transformers + their CUDA pins that may conflict with our backend's pins). §⑧ MCP table updated: `pdf_extract` is no longer a FastMCP wrap of `datalab-to/marker` in PaperHub's venv — instead it's an **isolated Docker container running Marker's own environment** (e.g., `docker run -p 8080:8080 datalab-to/marker-api:latest` or the equivalent). PaperHub's backend calls it over plain HTTP; the call site still goes through the MCP scope-checker + tracer (the MCP server can itself be a thin in-process FastMCP that forwards to the Marker HTTP container, so the audit trail is preserved). The container architecture decouples Marker's CUDA / Python version from PaperHub's. No FR or NFR removals. |
| v1.11 | 2026-05-17 | **Phase A actual-completion record.** Documents what shipped in Phase A vs what defers cleanly to Phase B, so the SRS stays honest about implementation reality. Phase A delivered: (i) full §1.1 Tier-1 LaTeX import (unpacked e-print archive + flattened LaTeX) via `arxiv_latex` MCP + `arxiv.Result.download_source()` for the raw `.tar.gz`; (ii) §1.1 Tier-3 markdown fallback via existing `arxiv` MCP; (iii) RAG over indexed LaTeX chunks; (iv) Router (binary `paper_qa` | `chitchat`); (v) Research Agent; (vi) `/chat` SSE with full audit trail; (vii) chat UI; (viii) Tool-Call Tracer with secret + binary redaction; (ix) MCP scope-checker with CVE-2025-53109 regression guard; (x) `.env`-driven multi-provider LiteLLM config (Anthropic, OpenAI, Gemini); (xi) live e2e tests against real arxiv + real Gemini covering import-with-figures + chat with citations. Phase A explicitly defers to Phase B per user direction: (a) **Marker container** (Tier 2 of §1.1 ladder) — scaffolded via `Settings.marker_url`/`marker_enabled=False`, no live wire; (b) **agentic search → read → decide → download** flow (new `paper_import` Router intent + ImportAgent calling `arxiv.search_papers` → `arxiv.get_abstract` → internal import) — current Phase A only supports direct `POST /papers/import {arxiv_id}`; (c) all remaining Phase B / C deliverables in the design §4 phase table (full 6-intent Router, SQL Agent, Report Agent + slides, relation analysis, projects, eval harness). No FR or NFR removals; **all Phase A deferrals are part of v1, not v2** — they just land in Phase B per the build-order phasing. The Acceptance criteria numbered against the v1 release continue to apply; Phase A is the "Acceptance #3 + part of #1 + part of #8 + part of #10/#11" slice (see traceability in the companion design doc). |
...
1. **Source-format fidelity at import time — three tiers, falls through on failure.** Extractor selection is a strict priority ladder. The import pipeline tries each tier in order and only falls through to the next on a hard failure of the current one. Crucially, each tier is functionally different in what it preserves:

   - **Tier 1 — Raw paper source code (LaTeX).** *Lossless.* The same path paper2slides-plus uses. Preserves authors' original equations, figures (with `\includegraphics`), tables, citation labels, and macro definitions byte-for-byte. **The primary Tier-1 artifact is the unpacked e-print archive, NOT just the flattened LaTeX text** — the slide pipeline (FR-05 / FR-07) needs the figures (`.png` / `.pdf` / `.eps`), the bibliography (`.bib`), the style files (`.sty` / `.cls`), and any `\include{}`'d chapter files; flattened LaTeX alone is not enough to recompile.
     - **arXiv ID** → download the raw e-print `.tar.gz` from `https://arxiv.org/e-print/<id>` (via the official `arxiv` Python library's `Result.download_source(...)`), unpack to `workspace_root/papers/<id>/source/`. In parallel, call [`takashiishida/arxiv-latex-mcp`](https://github.com/takashiishida/arxiv-latex-mcp)::`get_paper_prompt` (which itself wraps `arxiv-to-prompt`) for the flattened LaTeX text — save that to `workspace_root/papers/<id>/source.flattened.tex` and feed it to the chunker for RAG indexing. The unpacked `source/` is the artifact the slide pipeline reads from; the flattened `.flattened.tex` is the RAG-side rendition.
     - **DOI → arXiv preprint** → resolve to arXiv ID, then the arXiv path above.
     - **DOI → publisher with LaTeX source** → publisher-specific (not implemented in v1; degrades to Tier 2).
     - **Reference**: paper2slides-plus's [`src/arxiv_utils.py`](../../../reference/paper2slides-plus/src/arxiv_utils.py) `copy_image_assets_from_cache` demonstrates the canonical figure-preservation pattern PaperHub MUST inherit.
   - **Tier 2 — Marker (equation-preserving Markdown).** *High-fidelity fallback.* [`datalab-to/marker`](https://github.com/datalab-to/marker) converts PDF → Markdown while **preserving equations as inline LaTeX math** (`$...$`, `$$...$$`), keeping table structure as Markdown tables, and saving figures as referenced images. The structure is post-extraction so macros and exact LaTeX environments aren't recovered, but for the downstream slide pipeline (FR-05 / FR-07) and for RAG (FR-03), the equations survive — which is the whole point.
     - Used when Tier 1 fails (no LaTeX e-print available — rare for arXiv, common for older papers, withdrawn papers, non-arXiv DOIs).
     - Always used for **user-uploaded local PDFs** (Tier 1 isn't available).
     - Output saved to `workspace_root/papers/<id>/marker/` (`.md` + figure images).
     - Heavyweight (~2–4 GB ML models on first run, GPU-friendly but CPU-capable). Installed as optional `[pdf]` extra.
   - **Tier 3 — Raw text extraction.** *Lossy last resort.* `pdfminer` / `PyMuPDF` raw text, or HTML→Markdown conversion (e.g., the body-text route of `blazickjp/arxiv-mcp-server`). Equations collapse to broken unicode, figures vanish, tables become whitespace-mangled rows. Used **only** when both Tier 1 and Tier 2 are unavailable (Marker not installed AND no LaTeX). Sets `papers.notes_md='low_fidelity_extraction'` so downstream consumers (slide pipeline especially) know the artifact is degraded — the slide pipeline MAY refuse to operate on Tier-3 inputs to avoid producing broken Beamer.

   **Fail-loud rule:** if all three tiers fail (e.g., the arXiv ID is invalid AND no PDF available), the import fails with a clear error message naming each tier that was tried and why. We never silently produce an empty paper row.

   **Ladder enforcement:** the import pipeline must record which tier was used in `papers.notes_md` (or a dedicated `papers.extraction_tier` column) so the slide pipeline, RAG retriever, and audit log can act on it. Skipping straight to a lower tier without trying the higher one is prohibited.

   **Rationale for inheriting paper2slides-plus's LaTeX preference:** the predecessor learned that LaTeX-source slides are dramatically higher quality than PDF-extracted slides — equations render perfectly, figures stay vector, citation links work. PaperHub inherits this preference verbatim. Marker (Tier 2) exists to handle the cases the predecessor couldn't (user uploads, withdrawn papers, non-arXiv DOIs) while still preserving the equations the slide pipeline depends on. Tier 3 exists only so the import doesn't crash when neither of the better options is available — but it's flagged and the slide pipeline is allowed to refuse it.

   **NOT a body-text extractor:** GROBID is used for structured *reference list* extraction (FR-02), which is what it's actually good at; it is never used to produce the primary body artifact.
...
| `pdf_extract` | PDF path must be inside `workspace_root`; per-call timeout (configurable, default 5 min for large papers); HTTP endpoint pinned to the configured Marker container URL (default `http://localhost:8080`) | High-fidelity PDF → Markdown + figure images + equation captures. Used for local-PDF imports and DOI imports where no LaTeX is available. Per §1.1 First Principle — the only sanctioned PDF body-text extractor in PaperHub. | **Containerized service + thin MCP wrap.** Marker runs as an **isolated Docker container** with its own environment (PyTorch + transformers + CUDA pins that would conflict with PaperHub's backend venv if installed in-process). Container image: `datalab-to/marker-api:latest` (or build from `datalab-to/marker`'s `Dockerfile`); typically exposed on port 8080. PaperHub's `paperhub/mcp/tools/pdf_extract_server.py` is a thin (~50 LoC) FastMCP server that forwards each invocation to the Marker container's HTTP API and returns the response — so audit / scope-checking still flow through `mcp/client.py`. If the container is absent (URL unreachable), the import path degrades to Tier 3 with `papers.notes_md='low_fidelity_extraction'`. **Phase A scope:** the MCP wrap + Settings field exist; Marker container deployment + the actual Phase-A test of this path is Phase B. |
