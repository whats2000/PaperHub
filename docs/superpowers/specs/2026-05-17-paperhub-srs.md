...
1. **Source-format fidelity at import time — three tiers, falls through on failure.** Extractor selection is a strict priority ladder. The import pipeline tries each tier in order and only falls through to the next on a hard failure of the current one. Crucially, each tier is functionally different in what it preserves:
   ...
   - **Tier 1 — Raw paper source code (LaTeX).** *Lossless.* The same path paper2slides-plus uses. Preserves authors' original equations, figures (with `\includegraphics`), tables, citation labels, and macro definitions byte-for-byte.
     - **arXiv ID** → [`takashiishida/arxiv-latex-mcp`](https://github.com/takashiishida/arxiv-latex-mcp) (fetches the e-print `.tar.gz`, returns flattened LaTeX text).
     - **DOI → arXiv preprint** → resolve to arXiv ID, then arxiv-latex-mcp.
