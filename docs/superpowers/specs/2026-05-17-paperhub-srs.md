...
1. **Source-format fidelity at import time.** The primary stored artifact is always the **highest-fidelity source the publisher exposes**, chosen by the import-source-format matrix:

   | Source | Primary artifact | Extractor | Secondary artifact (for RAG) |
   | --- | --- | --- | --- |
   | arXiv | `.tex` / unpacked `.tar.gz` e-print | [`takashiishida/arxiv-latex-mcp`](https://github.com/takashiishida/arxiv-latex-mcp) | Plain-text rendition of the LaTeX (Markdown derived from `\section` structure) |
   | Local PDF upload | the PDF itself | [`datalab-to/marker`](https://github.com/datalab-to/marker) — high-fidelity PDF→Markdown that preserves equations, tables, figures | Marker's Markdown output + extracted figure images + equation captures, saved under `workspace_root/papers/<id>/marker/` |
   | DOI lookup | publisher's LaTeX if available; otherwise their PDF (processed via Marker) | `arxiv_latex` if the DOI resolves to an arXiv preprint, else Marker | as above |

   Lower-fidelity converters (`pdfminer` raw text, HTML→Markdown, the body-text path of `arxiv-mcp-server`) are **prohibited as primary artifact sources** — they silently destroy equations, figure references, table structure, and citation labels, all of which the slide pipeline (FR-05 / FR-07) needs intact. GROBID continues to be used for structured *reference list* extraction (FR-02), which is what it's actually good at; it is not a body-text extractor in PaperHub.
