...
**v2.6 rescinds this deferral.** Keeping the dual dispatch path (in-process `papers.*`, MCP-client `web.*`) re-introduces the exact "manual tool" pattern the v2.5 course-correction set out to eliminate, just dressed up. v2.6 pulls the `paperhub-papers` FastMCP server into Plan C v2.5 scope: a FastMCP sub-app is mounted on the existing FastAPI app at `/mcp` (no extra process, no second port), and the Research Agent's `MCPClient` reaches it over HTTP loopback. Every tool the agent has — `papers.*` and `web.*` — now flows through one dispatch path. The external `paperhub.*` MCP surface promised in §III-6's last paragraph (Claude Desktop / Cursor reach) comes along for free at the same URL. Plan E's residual scope is now only the sqlite MCP server. See §III-6's `paperhub-papers` row above for the operational shape; Plan C v2.5 follow-up Task v2.5-3 builds it.
